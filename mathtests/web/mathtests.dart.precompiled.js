...
["", "mathtests.dart", , F, {
  "": "",
  main: [function() {
    var t1, bEancheck, t2, bISBNcheck, bHammingCalculate, bHammingCorrect;
    ...
    F.printresize();
    bEancheck = document.querySelector("#eancheck");
    bEancheck.toString;
    t1 = C.EventStreamProvider_click._eventType;
    t2 = H.setRuntimeTypeInfo(new W._ElementEventStreamImpl(bEancheck, t1, false), [null]);
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(new F.main_closure0()), t2._useCapture), [H.getTypeArgumentByIndex(t2, 0)])._tryResume$0();
    bISBNcheck = document.querySelector("#isbncheck");
    bISBNcheck.toString;
    t2 = H.setRuntimeTypeInfo(new W._ElementEventStreamImpl(bISBNcheck, t1, false), [null]);
    H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(new F.main_closure1()), t2._useCapture), [H.getTypeArgumentByIndex(t2, 0)])._tryResume$0();
    bHammingCalculate = document.querySelector("#hammingcalculate");
    bHammingCalculate.toString;
    t2 = H.setRuntimeTypeInfo(new W._ElementEventStreamImpl(bHammingCalculate, t1, false), [null]);
  ...
  printresize: function() {
    ...
    document.querySelector("#innerwidth").textContent = "Inner width: " + H.S(innerwidth);
    document.querySelector("#innerheight").textContent = "Inner height: " + H.S(innerheight);
  },
  eancheck: function() {
    var inp, t1, t2, onevalues, i, t3, threevalues;
    inp = J.get$value$x(document.querySelector("#eaninput"));
    for (t1 = J.getInterceptor$asx(inp), t2 = inp.length, onevalues = 0, i = 0; i < t1.get$length(inp); i += 2) {
      if (i >= t2)
        return H.ioore(inp, i);
      t3 = H.Primitives_parseInt(inp[i], null, new F.eancheck_closure());
      if (typeof t3 !== "number")
        return H.iae(t3);
      onevalues += t3;
    }
    for (threevalues = 0, i = 1; i < t2; i += 2) {
      t1 = J.$mul$n(H.Primitives_parseInt(inp[i], null, new F.eancheck_closure0()), 3);
      if (typeof t1 !== "number")
        return H.iae(t1);
      threevalues += t1;
    }
    J.replaceWith$1$x(document.querySelector("#validity"), F.valcheck(onevalues + threevalues, 10, "validity", 13, inp));
  },
  isbncheck: function() {
    var t1, inp, t2, t3, checksum, fact, i, t4;
    t1 = new F.isbncheck_numparse();
    inp = J.get$value$x(document.querySelector("#isbninput"));
    for (t2 = J.getInterceptor$asx(inp), t3 = inp.length, checksum = 0, fact = 10, i = 0; i < t2.get$length(inp); ++i) {
      if (i >= t3)
        return H.ioore(inp, i);
      t4 = J.$mul$n(t1.call$1(inp[i]), fact);
      if (typeof t4 !== "number")
        return H.iae(t4);
      checksum += t4;
      --fact;
    }
    J.replaceWith$1$x(document.querySelector("#isbnvalidity"), F.valcheck(checksum, 11, "isbnvalidity", 10, inp));
  },
  valcheck: function(checksum, mod, id, $length, input) {
    var ret = W._ElementFactoryProvider_createElement_tag("p", null);
    J.set$id$x(ret, id);
    if (C.JSNumber_methods.$mod(checksum, mod) === 0 && J.get$length$asx(input) === $length) {
      ret.textContent = "Valid";
      J.set$color$x(ret.style, "#669900");
    } else {
      ret.textContent = "Invalid";
      J.set$color$x(ret.style, "#CC0000");
    }
    return ret;
  },
  calcParity: function() {
    ...
    t1 = {};
    t1.sParities_1 = "";
    message = H.interceptedTypeCast(document.querySelector("#hammingmessage"), "$isInputElement").value;
    H.IterableMixinWorkaround_forEach($.get$circles(), new F.calcParity_add(t1, message));
    H.interceptedTypeCast(document.querySelector("#hammingparity"), "$isInputElement").value = t1.sParities_1;
  },
  getCorrected: function(inp) {
    var t1, errors, corrected, t2;
    t1 = {};
    errors = H.setRuntimeTypeInfo([], [J.JSInt]);
    t1.total_0 = null;
    t1.index_1 = 4;
    H.IterableMixinWorkaround_forEach($.get$circles(), new F.getCorrected_check(t1, inp, errors));
    t1 = errors.length;
    switch (t1) {
      case 1:
        if (0 >= t1)
          return H.ioore(errors, 0);
        corrected = C.JSString_methods.substring$2(inp, 0, errors[0]);
        if (0 >= errors.length)
          return H.ioore(errors, 0);
        t1 = errors[0];
        if (t1 >= inp.length)
          return H.ioore(inp, t1);
        t1 = H.Primitives_parseInt(inp[t1], null, null);
        if (typeof t1 !== "number")
          return H.iae(t1);
        corrected += C.JSNumber_methods.toString$0(1 - t1);
        if (0 >= errors.length)
          return H.ioore(errors, 0);
        corrected += C.JSString_methods.substring$1(inp, errors[0] + 1);
        break;
      case 2:
        if (0 >= t1)
          return H.ioore(errors, 0);
        t2 = errors[0];
        if (1 >= t1)
          return H.ioore(errors, 1);
        switch (t2 + errors[1]) {
          case 9:
            if (0 >= inp.length)
              return H.ioore(inp, 0);
            t1 = H.Primitives_parseInt(inp[0], null, null);
            if (typeof t1 !== "number")
              return H.iae(t1);
            corrected = C.JSNumber_methods.toString$0(1 - t1) + C.JSString_methods.substring$1(inp, 1);
            break;
          case 11:
            corrected = C.JSString_methods.substring$2(inp, 0, 2);
            if (2 >= inp.length)
              return H.ioore(inp, 2);
            t1 = H.Primitives_parseInt(inp[2], null, null);
            if (typeof t1 !== "number")
              return H.iae(t1);
            corrected = corrected + C.JSNumber_methods.toString$0(1 - t1) + C.JSString_methods.substring$1(inp, 3);
            break;
          case 10:
            t1 = inp.length;
            if (0 >= t1)
            ...
            corrected = inp[0];
            if (1 >= t1)
              return H.ioore(inp, 1);
            t1 = H.Primitives_parseInt(inp[1], null, null);
            if (typeof t1 !== "number")
              return H.iae(t1);
            corrected = corrected + C.JSNumber_methods.toString$0(1 - t1) + C.JSString_methods.substring$1(inp, 2);
            break;
          default:
            corrected = inp;
        }
        break;
      case 3:
        corrected = C.JSString_methods.substring$2(inp, 0, 3);
        if (3 >= inp.length)
          return H.ioore(inp, 3);
        t1 = H.Primitives_parseInt(inp[3], null, null);
        if (typeof t1 !== "number")
          return H.iae(t1);
        corrected = corrected + C.JSNumber_methods.toString$0(1 - t1) + C.JSString_methods.substring$1(inp, 4);
        break;
      default:
        corrected = inp;
    }
  ...
  isbncheck_numparse: {
    ...
    call$1: function(x) {
      if (x.toUpperCase() === "X")
        return 10;
      return H.Primitives_parseInt(x, null, new F.isbncheck_numparse_closure());
    }
  },
  isbncheck_numparse_closure: {
    "": "Closure:11;",
    call$1: function(e) {
      return 0;
    }
  ...
  calcParity_add: {
    "": "Closure:18;box_1,message_2",
    call$1: function(indexes) {
      var t1, t2;
      t1 = {};
      t1.total_0 = 0;
      J.forEach$1$ax(indexes, new F.calcParity_add_closure(t1, this.message_2));
      t2 = this.box_1;
      t2.sParities_1 = t2.sParities_1 + C.JSNumber_methods.toString$0(C.JSNumber_methods.$mod(t1.total_0, 2));
    }
  },
  calcParity_add_closure: {
    "": "Closure:11;box_0,message_3",
    call$1: function($E) {
      var t1, t2, t3, total;
      t1 = this.box_0;
      t2 = t1.total_0;
      t3 = this.message_3;
      if ($E >>> 0 !== $E || $E >= t3.length)
        return H.ioore(t3, $E);
      t3 = H.Primitives_parseInt(t3[$E], null, null);
      if (typeof t3 !== "number")
        return H.iae(t3);
      total = t2 + t3;
      t1.total_0 = total;
  ...
  getCorrected_check: {
    "": "Closure:18;box_0,inp_1,errors_2",
    call$1: function(indexes) {
      var t1, t2, t3, t4;
      t1 = this.box_0;
      t1.total_0 = 0;
      t2 = this.inp_1;
      J.forEach$1$ax(indexes, new F.getCorrected_check_closure(t1, t2));
      t3 = t1.total_0;
      if (typeof t3 !== "number")
        return t3.$mod();
      t3 = C.JSNumber_methods.$mod(t3, 2);
      t4 = t1.index_1;
      if (t4 >= t2.length)
        return H.ioore(t2, t4);
      if (t3 !== H.Primitives_parseInt(t2[t4], null, null))
        this.errors_2.push(t1.index_1);
      t1.index_1 = t1.index_1 + 1;
    }
  },
  getCorrected_check_closure: {
    "": "Closure:11;box_0,inp_3",
    call$1: function($E) {
      var t1, t2, t3, total;
      t1 = this.box_0;
      t2 = t1.total_0;
...
Isolate.$lazy($, "circles", "circles", "get$circles", function() {
